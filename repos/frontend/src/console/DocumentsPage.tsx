...
import { listProducts, listDocuments, uploadDocument, getDocumentContent, previewDocumentUrl } from '../lib/api';
import type { Product, KnowledgeDocument } from '../lib/types';

const docTone: Record<string, 'ok' | 'warn' | 'danger' | 'mute'> = {
  ready: 'ok',
  parsing: 'warn',
  uploaded: 'warn',
  failed: 'danger',
...
export default function DocumentsPage() {
  ...
  const processing = documents.filter((d) => d.status === 'uploaded' || d.status === 'parsing').length;
  const failed = documents.filter((d) => d.status === 'failed').length;

  const statusOptions = ['uploaded', 'parsing', 'ready', 'failed', 'archived'];
  ...
  return (
    <div className="enter">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard label={t('status.ready')} value={ready} tone="ok" />
        <StatCard label={t('common.processing')} value={processing} tone="ai" />
        <StatCard label={t('status.failed')} value={failed} tone="safety" />
      </div>
            ...
            <tbody>
              {filtered.map((d) => (
                <tr key={d.id} className="border-t border-line transition hover:bg-brand-soft/40">
                  ...
                  <td className="px-4 py-3 text-ink2">{d.language}</td>
                  <td className="px-4 py-3 text-ink2">{d.region}</td>
                  <td className="px-4 py-3 text-ink2">{d.productName ?? '—'}</td>
                  <td className="px-4 py-3">
                    <Tag tone={docTone[d.status] ?? 'mute'}>{t(`status.${d.status}`)}</Tag>
                  </td>
                  <td className="px-4 py-3">
