...
export default function QrsPage() {
  ...
  const [form, setForm] = useState({ productModelId: '', productVariantId: '', initialFirmwareVersion: '', batch: '', serialNumber: '' });
  ...
  const create = async () => {
    ...
    setShowNew(false);
    setForm({ productModelId: '', productVariantId: '', initialFirmwareVersion: '', batch: '', serialNumber: '' });
    setShowToken(item);
    load();
  };
    ...
    <div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <select className="w-full rounded border border-line px-3 py-2 text-sm" value={form.productVariantId}
              ...
              {variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.hardwareRevision || 'Unspecified'} · {variant.region}{variant.sku ? ` · ${variant.sku}` : ''}</option>)}
            </select>
          </div>
          <div>
            <label className="mb-1 block text-xs text-ink2">Initial firmware version</label>
            <Input value={form.initialFirmwareVersion} onChange={(e) => setForm({...form, initialFirmwareVersion: e.target.value})}/>
          </div>
          <div>
      ...
      <Modal open={!!showToken} title={t('qrs.tokenTitle')} onClose={() => setShowToken(null)}>
        <div className="space-y-3">
          ...
          <div className="break-all rounded-lg border border-line bg-slate-50 p-3 font-mono text-xs">{showToken?.token}</div>
