...
const publicApi = async (path: string, init: RequestInit = {}) => {
  const response = await fetch(`/public${path}`, init);
  if (!response.ok) throw new Error((await response.json().catch(() => null))?.message ?? '服务暂不可用');
  return response.status === 204 ? null : response.json();
};
...
function SupportApp({ qrToken }: { qrToken: string }) {
  ...
  const begin = async () => { try { const item = await publicApi('/conversations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ qrToken, language }) }); setConversationId(item.id); setNotice('设备已识别。请描述问题或输入错误码。'); } catch (e) { setNotice((e as Error).message); } };
  const send = async () => { if (!conversationId || !input.trim()) return; const text = input; setInput(''); try { let message; if (attachment) { const body = new FormData(); body.append('content', text); body.append('file', attachment); const response = await fetch(`/public/conversations/${conversationId}/attachments`, { method: 'POST', body }); if (!response.ok) throw new Error((await response.json().catch(() => null))?.message ?? '图片上传失败'); const data = await response.json(); message = { id: data.messageId, content: text }; setAttachment(null); } else { message = await publicApi(`/conversations/${conversationId}/messages`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: text, errorCode: /^\w[\w-]{2,}$/.test(text) ? text : undefined }) }); } const answer = await publicApi(`/conversations/${conversationId}/answers`, { method: 'POST' }); setMessages([...messages, message, { id: `answer-${message.id}`, content: answer.content, citations: answer.citationChunkIds }]); } catch (e) { setInput(text); setNotice(`发送失败：${(e as Error).message}`); } };
  const feedback = async (resolved: boolean) => { if (!conversationId) return; try { await publicApi(`/conversations/${conversationId}/feedback`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ resolved }) }); setNotice(resolved ? '感谢反馈。' : '我们会协助你转交人工支持。'); } catch (e) { setNotice((e as Error).message); } };
  const changeProduct = async () => { const productModelId = prompt('请输入正确的产品 ID'); if (!productModelId || !conversationId) return; try { await publicApi(`/conversations/${conversationId}/product-context`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productModelId }) }); setNotice('产品上下文已更新，之前的诊断步骤已结束。'); } catch (e) { setNotice((e as Error).message); } };
  return <main className="support"><section><small>HARDWARE AI SUPPORT</small><h1>设备支持</h1><p>{notice}</p><label>语言 <select value={language} onChange={(e) => setLanguage(e.target.value)}><option value="en-US">English</option><option value="zh-CN">简体中文</option></select></label>{!conversationId && <button className="primary" onClick={begin}>开始支持</button>}<div className="thread">{messages.map(m => <p key={m.id}>{m.content}{m.citations?.length ? <small> 来源：{m.citations.join(', ')}</small> : null}</p>)}</div>{conversationId && <><textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="描述问题或粘贴错误码" /><input type="file" accept="image/png,image/jpeg" onChange={(e) => setAttachment(e.target.files?.[0] ?? null)} />{attachment && <small>已选择：{attachment.name}</small>}<button className="primary" onClick={send}>发送</button><p><button onClick={changeProduct}>更换产品</button><button onClick={() => feedback(true)}>已解决</button><button onClick={() => feedback(false)}>未解决</button></p></>}</section></main>;
}
