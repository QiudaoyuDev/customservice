  ...
  const createProduct = async () => {
    ...
    try {
      await api('/products', {
        ...
        body: JSON.stringify({ family, model, displayName, region }),
      });
      await loadProducts();
    } catch (e) {
      setMessage((e as Error).message);
  ...
  return (
    <main>
      <aside>
        ...
        <nav>
          {(['products', 'documents', 'search'] as const).map((item) => (
            <button
              className={tab === item ? 'active' : ''}
              onClick={() => setTab(item)}
              key={item}
            >
              {item === 'products' ? '产品上下文' : item === 'documents' ? '知识文档' : '检索验证'}
            </button>
          ))}
        </nav>
      ...
      <section>
        ...
        {tab === 'documents' && (
          <article className="empty">
            <h2>知识文档</h2>
            <p>
              后端已提供 PDF、DOCX、PNG、JPEG 上传接口。下一步将接入解析任务状态和审核发布操作。
            </p>
          </article>
        )}
        {tab === 'search' && (
          <article className="empty">
            <h2>检索验证</h2>
            <p>发布索引与混合检索接口将在文档解析 Worker 完成后接入此页面。</p>
          </article>
        )}
      </section>
    </main>
