...
class SupportOrchestratorService {
    ...
    SupportOrchestratorService(RetrievalService retrieval, ConversationMessageRepository messages,
                               AnswerTraceRepository traces, AnswerCitationRepository citations, ModelConfigurationService models, ObjectMapper json, OperationalEventService events) {
        this.retrieval = retrieval; this.messages = messages; this.traces = traces; this.citations = citations; this.models = models; this.json = json; this.events = events;
    }
    ...
    Result answer(Conversation conversation, ConversationProductContext context, ConversationMessage question, Intent intent) {
        ...
        if (retrieved.conflictDetected()) return persist(conversation, question, intent, "CONFLICT", "Evidence conflicts; please contact support.", List.of(), start);
        if (retrieved.evidence().isEmpty()) return persist(conversation, question, intent, "NO_EVIDENCE", "I do not have enough verified information. Please contact support.", List.of(), start);
        // Safe template fallback: no raw whole-document dump and no instruction not backed by selected evidence.
        String summary = retrieved.evidence().stream().map(e -> "[C" + (retrieved.evidence().indexOf(e) + 1) + "] " + excerpt(e.excerpt())).collect(java.util.stream.Collectors.joining("\n"));
        try {
            var response = models.generate(conversation.tenantId(), systemPrompt(), "Question: " + question.content() + "\nEvidence:\n" + summary);
            if (response.isPresent()) {
                String answer = validateModelOutput(response.get().content(), retrieved.evidence().size());
                if (answer != null) return persist(conversation, question, intent, "MODEL", answer, retrieved.evidence(), start, response.get().configurationId());
            }
    ...
    private Result persist(Conversation conversation, ConversationMessage question, Intent intent, String outcome, String content,
        ...
        var trace = traces.save(new AnswerTrace(conversation.id(), assistant.id(), modelConfigurationId, intent.name(), outcome,
                java.util.concurrent.TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start), outcome, evidence.size(), evidence.size()));
        ...
        events.record(conversation.tenantId(), conversation.id(), "ANSWER_COMPLETED", java.util.Map.of("intent", intent.name(), "outcome", outcome, "citationCount", evidence.size(), "latencyMs", java.util.concurrent.TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start)));
        return new Result(content, evidence.stream().map(item -> item.documentTitle() + "#" + item.chunkId()).toList(), outcome);
    }

    private static String excerpt(String text) { return text == null ? "" : text.substring(0, Math.min(text.length(), 700)); }
    ...
    private String validateModelOutput(String value, int evidenceCount) throws java.io.IOException {
        ...
        Object answer = data.get("answer"); Object rawCitations = data.get("citations");
        if (!(answer instanceof String text) || text.isBlank() || !(rawCitations instanceof List<?> keys) || keys.isEmpty()) return null;
        for (Object key : keys) if (!(key instanceof String valueKey) || !valueKey.matches("C[1-" + evidenceCount + "]")) return null;
        return text;
    }
    private static String systemPrompt() { return "Return JSON only: {answer:string,citations:string[],followUpQuestion:string|null,resolutionCandidate:boolean,handoffRecommended:boolean}. Use only supplied Evidence, cite every factual conclusion with Cn, do not give disassembly, flashing or unsafe repair instructions."; }
