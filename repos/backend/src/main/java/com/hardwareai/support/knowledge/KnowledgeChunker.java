    ...
    List<KnowledgeChunk> split(KnowledgeRevision revision, KnowledgeDocument document) {
        ...
        for (String paragraph : paragraphs) {
            ...
            if (clean.startsWith("# ")) {
                if (current.length() > 0) {
                    chunks.add(
                        new KnowledgeChunk(revision.id(), chunks.size(), current.toString(), document.title(), titlePath, page,
                            page));
                    current.setLength(0);
                }
            ...
            if (current.length() > 0 && current.length() + clean.length() + 2 > MAX_CHARS) {
                chunks.add(
                    new KnowledgeChunk(revision.id(), chunks.size(), current.toString(), document.title(), titlePath, page, page));
                current.setLength(0);
            }
            if (clean.length() > MAX_CHARS && current.length() == 0) {
                for (int start = 0; start < clean.length(); start += MAX_CHARS) {
                    chunks.add(new KnowledgeChunk(revision.id(), chunks.size(),
                        clean.substring(start, Math.min(clean.length(), start + MAX_CHARS)), document.title(), titlePath, page,
                        page));
                }
            } else {
