...
final class ConclusionConflictDetector {
    ...
    boolean conflicts(List<String> excerpts) {
        ...
        return excerpts.stream().anyMatch(ConclusionConflictDetector::isPermissive)
            ...
            excerpts.stream().filter(ConclusionConflictDetector::isRestrictive).anyMatch(text -> lower(text).contains(anchor))
                && excerpts.stream().filter(ConclusionConflictDetector::isPermissive)
                .anyMatch(text -> lower(text).contains(anchor)));
    }
