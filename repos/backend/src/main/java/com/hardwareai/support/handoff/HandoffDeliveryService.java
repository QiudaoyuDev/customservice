...
class HandoffDeliveryService {
    ...
    HandoffDeliveryService(List<HumanSupportAdapter> adapters) {
    ...
    void deliver(HandoffRequest request) {
        ...
        for (var adapter : adapters) {
            try {
                ...
                if (result.delivered())
                    log.info("Handoff delivered handoff={} channel={} externalId={}", request.id(), adapter.channel(),
                        result.externalConversationId());
                else log.warn("Handoff delivery rejected handoff={} channel={} code={}", request.id(), adapter.channel(),
                    result.errorCode());
