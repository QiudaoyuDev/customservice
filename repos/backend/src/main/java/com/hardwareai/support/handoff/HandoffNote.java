...
class HandoffNote {
    @Id private UUID id;
    @Column(name = "handoff_id") private UUID handoffId;
    @Column(name = "author_id") private UUID authorId;
    @Column(columnDefinition = "text") private String content;
    @Column(name = "created_at") private Instant createdAt = Instant.now();

    protected HandoffNote() { }
    HandoffNote(UUID handoffId, UUID authorId, String content) { id = UUID.randomUUID(); this.handoffId = handoffId; this.authorId = authorId; this.content = content; }
    UUID id() { return id; }
