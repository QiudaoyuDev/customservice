...
@Service public class TroubleshootStateMachine {
 public Transition next(NodeType type,Risk risk,Reply reply,String yes,String no,String unknown,int failures){
  if(risk==Risk.HIGH||reply==Reply.REFUSE||failures>=2) return new Transition("HUMAN_ESCALATION",true);
  String node=switch(reply){case YES->yes;case NO->no;case UNKNOWN->unknown;case REFUSE->"HUMAN_ESCALATION";};
