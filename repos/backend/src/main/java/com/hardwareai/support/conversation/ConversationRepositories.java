...
interface AnswerTraceRepository extends JpaRepository<AnswerTrace, UUID> { List<AnswerTrace> findAllByConversationIdOrderByCreatedAtAsc(UUID conversationId); }

interface AnswerCitationRepository extends JpaRepository<AnswerCitation, UUID> { List<AnswerCitation> findAllByAnswerTraceIdIn(List<UUID> traceIds); }
interface AttachmentProcessingJobRepository extends JpaRepository<AttachmentProcessingJob, UUID> { java.util.Optional<AttachmentProcessingJob> findFirstByStatusOrderByCreatedAt(String status); }
interface AttachmentAnalysisRepository extends JpaRepository<AttachmentAnalysis, UUID> { java.util.Optional<AttachmentAnalysis> findByAttachmentId(UUID attachmentId); List<AttachmentAnalysis> findAllByAttachmentIdIn(List<UUID> attachmentIds); }
...
interface ConversationFlowStepRepository extends JpaRepository<ConversationFlowStep, UUID> { List<ConversationFlowStep> findAllByFlowSessionIdInOrderByCreatedAtAsc(List<UUID> flowSessionIds); }
